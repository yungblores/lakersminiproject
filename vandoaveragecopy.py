...
for i, col in enumerate(stat_cols):
    ...
    player = df_stat[df_stat['Name'] == 'Jarred Vanderbilt']
    player_percentile = percentileofscore(df_stat[col], player[col].iloc[0])
    print(f"Percentile of 'Jarred Vanderbilt' in {col}: {player_percentile}")
    ...
    print(f"\nAverage percentile of 'Jarred Vanderbilt' across all categories: {average_percentile}")
    ...
    fig.update_layout(
        title_text=f'Box-and-Whisker Plot of {col} with Jarred Vanderbilt Average',
        yaxis_title_text=f'{col} Scores',
    )

    # Save the figure to an HTML file
    pyo.plot(fig, filename=f'average{i+1}.html', auto_open=False)
